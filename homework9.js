...
function generate(){
    ...
    roundScores(averages_and_variances.variances);

    const average_scores = sortKeysAndValues(averages_and_variances.averages);
    const average_variances = sortKeysAndValues(averages_and_variances.variances);

    display_graph(average_variances.sortedKeys, average_variances.sortedValues);
...
function set_mean_and_variance(scores, averages_and_variances){
    const mean_and_variance = arithmetic_mean_and_variance(averages_and_variances.variances);

    document.getElementById("average_of_averages").textContent = "Average of variances: " + String(Math.round(mean_and_variance.mean * 1000) / 1000);
    ...
    document.getElementById("variance_of_averages").textContent = "Variance of variances: " + String(mean_and_variance.variance);
